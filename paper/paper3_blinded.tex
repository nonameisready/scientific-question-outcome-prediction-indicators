...
structural-prior question posed in~\cite{paper2}, we further show
...
\emph{does} \cite{si2024}. The backtesting protocol of~\cite{paper2} introduced
...
\begin{enumerate}
  ...
  \item \textbf{No single generator.} Predictors learned from one
    ...
    use four sources --- an evidence-tension mining system, a direct-LLM
...
An earlier framework~\cite{paper1} built an evidence-graph system that generates
questions from cross-paper observational tensions. The
protocol paper~\cite{paper2} defined the backtesting benchmark and showed, in its
...
agreement with other models --- a practice the protocol paper's seven-rater study shows
...
Ten frozen evidence-graph questions from~\cite{paper1} join the dataset as a
...
direct-LLM, 196 future-work, 200 controls, 10 from~\cite{paper1}. Every non-FRB cell
...
ceiling the sealed v1.1 release~\cite{paper2} established for outcome taxonomies of this kind:
...
($\kappa=0.17$--$0.26$). We therefore adopt the protocol paper's reporting
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lc}
    ...
    Evidence-graph~\cite{paper1} ($n=10$)   & 0.20 \\
    \bottomrule
  \end{tabular}
\end{table}
...
(Section~\ref{sec:limitations}). the sealed v1.1 release~\cite{paper2} measures this channel
...
the honest base rate of real open problems. The ten evidence-graph questions~\cite{paper1}
--- all engaged by future literature under the protocol paper's adjudicated pilot
...
elsewhere). the protocol paper's own cross-instance anchor shows the same
...
A cautionary card from the imported~\cite{paper1} questions: \emph{``To what extent have
...
LLM-mediated pipeline, including this one and that of~\cite{paper1}, needs phrasing
audits of exactly this kind --- The sealed v1.1 release~\cite{paper2} released curation log
...
The protocol paper~\cite{paper2} poses the question its architecture needs
...
\begin{enumerate}
  ...
  \item \textbf{Explicit contradiction buys engagement; methodological
    ...
    nothing (0/61) --- consistent with the protocol paper's finding that questions
  ...
  \item \textbf{Quantification and persistence mark the refutation
    ...
    exactly what the protocol paper's protocol predicts: a persistent, quantified,
...
exactly the machine-scale structural search \cite{paper2} proposes:
...
retrieval misses engagement with object-level questions, as the evidence-graph
subset demonstrates (2/10 here vs.\ 10/10 under the protocol paper's full-text
...
be read as lower bounds. the sealed v1.1 release~\cite{paper2} attacks the same confound from
...
differ only in the remedy --- adjust the metric (\cite{paper2}) versus
...
defensible foundation. The sealed v1.1 release~\cite{paper2} generator decomposition and
...
model) and the protocol paper's decomposition are complementary defenses against
...
\begin{itemize}
  ...
  \item \textbf{LLM judges, not humans.} Label reliability is estimated
    ...
    lenient). The sealed v1.1 release~\cite{paper2} seven-rater study reframes what such
...
forward-looking test is already armed: The sealed v1.1 release~\cite{paper2} prospective
